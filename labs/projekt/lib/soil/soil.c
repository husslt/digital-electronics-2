...
uint16_t getTankLevelPercentage() {
    ...
    float percent;              // percentage calculation handling
    uint8_t pct;                // return value
    uint16_t max = 970;         // 0-1024 ... 0-5V, set maximum level of ADC, which will equal to 100 %, e.g. max voltage is 2,5V, then value is 2,5/5 * 1024 = 
    uint16_t min = 700;         // 0-1024, set minimum voltage corresponding to 0 %
    ADMUX &= ~(1<<REFS0); ADMUX |= (1<<REFS0); // Set reference Vref to AVCC, REFS1 = 0, REFS0 = 1
